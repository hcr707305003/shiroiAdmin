...
function initUploadFile(field, fileId = '', fileType = 'image') {
    ...
    $fileDom.fileinput({
        ...
        dropZoneClickTitle: '(或点击下方选择按钮)',
        initialPreviewConfig: type[fileType] || [],
        fileActionSettings: {
            showDrag: false,
            showDownload: false,
        },
        ...
        if (response.code === 200 && response.initialPreview !== undefined) {
            ...
            $('#' + field+"_name").val(data['files'][0]['name']);

        }
        try{
            eval(data['extra']['file_field'] + '_after_upload('+JSON.stringify(response)+')');
        } catch (e) {}
    });
}
