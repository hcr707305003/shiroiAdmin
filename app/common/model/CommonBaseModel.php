...
class CommonBaseModel extends Model
{
    ...
    //更新字段方法
    public array $updateAttrFunction = [];
    //更新字段
    public array $updateAttrParam = [];
    //更新的数据
    public array $update = [];
     ...
     * @throws DbException
     */
    public function scopeUpdate(Query $query, array $where, array $update)
    {
        $this->update = $update;
        foreach ($this->updateAttrParam as $k => $v) {
            if(isset($this->update[$v])) {
                $this->update[$v] = $this->{$this->updateAttrFunction[$k]}($update[$v]);
            }
        }
        $query = $query->where($where)->findOrEmpty();
        if($query->isExists()) {
            return $query->save($this->update);
        }
        return false;
    }
